...
logging.info("t The format of your input is legal, now loading to next step...")
...
# 标签个数为15个
tf.flags.DEFINE_integer("num_classes", 15, "Number of labels (depends on the task)")
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if FLAGS.train_or_restore == 'R':
                ...
                logger.info("t The format of your input is legal, now loading to next step...")
                out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", MODEL))
                logger.info("t Writing to {0}\n".format(out_dir))
            else:
            ...
            for batch_train in batches_train:
                ...
                if current_step % FLAGS.checkpoint_every == 0:
                    ...
                    logger.info("t Saved model checkpoint to {0}\n".format(path))
                if current_step % num_batches_per_epoch == 0:
                    current_epoch = current_step // num_batches_per_epoch
                    logger.info("t Epoch {0} has finished!".format(current_epoch))
