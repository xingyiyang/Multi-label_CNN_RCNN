    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            test_F_ts = f1_score(y_true=np.array(true_onehot_labels),
                                 y_pred=np.array(predicted_onehot_labels_ts), average='micro')
            test_hammloss = hamming_loss(y_true=np.array(true_onehot_labels),
            ...
            test_subsetacc = accuracy_score(y_true=np.array(true_onehot_labels),
                                            y_pred=np.array(predicted_onehot_labels_ts))
            test_lrap = label_ranking_average_precision_score(y_true=np.array(true_onehot_labels),
                                                              y_score=np.array(predicted_onehot_scores))
            ...
            logger.info("Predict by threshold: hammingloss {0:g}, rocauc {1:g}, subsetacc {2:g}, labelrap {3:g}"
                        .format(test_hammloss, test_rocauc, test_subsetacc, test_lrap))
